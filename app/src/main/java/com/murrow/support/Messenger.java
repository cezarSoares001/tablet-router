...
public class Messenger implements Runnable {
	...
	public void getObjectReferences(Factory factory){
		...
		FIB = factory.getLRPDaemon().getForwardingTable(); // forwarding information base, used to populate the active names list.
		layer3Daemon = factory.getLL3Daemon(); // layer 3 daemon, used to send messages.
	...
	private void resetSpinner(){
		validNetworkList = FIB.getRoutes(); // get active nodes from forwarding information table.
		...
		for (RouteTableEntry entry : validNetworkList){
			if (entry.getNetworkDistancePair().getNetwork().compareTo(-1) > 0 && entry.getNetworkDistancePair().getNetwork().compareTo(16) < 0)
					spinnerAdapter.add(allNamesAndNetworks.get(entry.getNetworkDistancePair().getNetwork()-1)); // network numbers go from 1-15, array elements from 0-14
		}
